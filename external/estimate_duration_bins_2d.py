...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        help="Path to one or more pretrained tokenizers.",
    )
    parser.add_argument(
        "-a",
        "--langs",
    ...
    parser.add_argument(
        "--text-field", default="text", help="The key in manifests to read transcripts from."
    )
    parser.add_argument(
        "--lang-field", default="lang", help="The key in manifests to read language from."
    )
    parser.add_argument(
        "-n",
        "--num_examples",
    ...
    if not quiet:
        ...
        print(
            pd.Series(sizes).describe(
                percentiles=[0.01, 0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.99, 0.995, 0.999]
            )
        )
...
def load_tokenizer_from_pretrained_model(model_id: str) -> TokenizerSpec:
    from nemo.collections.asr.models import ASRModel

    logger.info(f"Loading tokenizer from pretrained model: {model_id}...")

    model = ASRModel.from_pretrained(model_id)
    tokenizer = model.tokenizer

    logger.info(f"Tokenizer: {type(tokenizer)} loaded.")
...
def main():
    ...
    elif args.tokenizer_from_pretrained_model is not None:
        tokenizer = load_tokenizer_from_pretrained_model(args.tokenizer_from_pretrained_model)
