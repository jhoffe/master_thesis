    ...
    * python oomptimizer.py --module-name nemo.collections.asr.models.EncDecMultiTaskModel \
    ...
    for _ in range(2 if ddp else 1):
        if pretrained_name is not None:
            assert config_path is None and module_name is None, (
    ...
    optimizer, _ = model.setup_optimization(
        {"name": optimizer_name, "lr": 1e-7, "weight_decay": 0.0}
    )
    ...
    if is_2d_bucketing:
        ...
        final_profile = [
            ["[" + ",".join(map(str, b)) + "]", bs] for (b, _, __), bs in profile.items()
        ]
    else:
    ...
    click.secho("The profile was created with the following settings:")
    ...
    click.secho(
        "\tbucket_duration_bins=[" + ",".join(str(seqlen) for seqlen, bs in final_profile) + "]",
        bold=True,
    )
    click.secho(
