...
class LhotseAudioToTargetDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, cuts: CutSet) -> dict[str, torch.Tensor]:
        ...
        retained_cuts = [
            cut._first_non_padding_cut if isinstance(cut, MixedCut) else cut
            for cut in retained_padded_cuts
        ]
        ...
        if _key_available(retained_cuts, self.TARGET_KEY):
            ...
            tgt_audio, tgt_audio_lens = collate_audio(
                retained_cuts, recording_field=self.TARGET_KEY
            )
            ans.update(target_signal=tgt_audio, target_length=tgt_audio_lens)
        if _key_available(retained_cuts, self.REFERENCE_KEY):
            # TODO: use fault_tolerant=True for robust loading of target
            ref_audio, ref_audio_lens = collate_audio(
                retained_cuts, recording_field=self.REFERENCE_KEY
            )
    ...
    if isinstance(path_or_paths, list):
        ...
        for p in path_or_paths:
            ...
            sources.append(
                AudioSource(
                    type="file",
                    channels=list(range(cur_channel_idx, cur_channel_idx + i.channels)),
                    source=p,
                )
            )
            cur_channel_idx += i.channels
        assert all(i.samplerate == infos[0].samplerate for i in infos[1:]), (
...
def create_array(path: str) -> Array:
    assert path.endswith(".npy"), (
        f"Currently only conversion of numpy files is supported (got: {path})"
    )
    arr = np.load(path)
    ...
    with CutSet.open_writer(output_manifest) as writer:
        for item in load_jsonl(input_manifest):
            ...
            recording = create_recording(
                get_full_path(audio_file=item_input_key, manifest_file=input_manifest)
            )
            cut = recording.to_cut().truncate(
                duration=item.pop("duration"), offset=item.pop("offset", 0.0)
            )
            ...
            if (channels := item.pop(INPUT_CHANNEL_SELECTOR, None)) is not None:
                if cut.num_channels == 1:
                    assert len(channels) == 1 and channels[0] == 0, (
            ...
            if target_key in item:
                ...
                _as_relative(
                    cut.target_recording, item_target_key, enabled=not force_absolute_paths
                )
            ...
            if reference_key in item:
                ...
                _as_relative(
                    cut.reference_recording, item_target_key, enabled=not force_absolute_paths
                )
...
def _as_relative(recording: Recording, paths: list[str] | str, enabled: bool) -> None:
    ...
    if isinstance(paths, str):
        paths = [paths]
    assert len(recording.sources) == len(paths), (
