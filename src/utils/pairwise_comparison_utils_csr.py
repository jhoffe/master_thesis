...
FORMAT_DICT = {
    ('canary-finetune_spec-aug_speed-perturbations', 'parakeet-finetune_spec-aug'): 'Pre-FT',
    ('canary-finetune_SA_SP_ll', 'parakeet-finetune_SA_ll'): 'FT',
    ('canary-finetune_SA_SP_ll_SA', 'parakeet-finetune_SA_ll_SA'): 'FT+SA',
    ('canary-finetune_SA_SP_ll_PS', 'parakeet-finetune_SA_ll_PS'): 'FT+PS',
    ('canary-finetune_SA_SP_ll_SP', 'parakeet-finetune_SA_ll_SP'): 'FT+SP',
    ('canary-finetune_SA_SP_ll_SA_PS', 'parakeet-finetune_SA_ll_SA_PS'): 'FT+SA+PS',
    ('canary-finetune_SA_SP_ll_SA_SP', 'parakeet-finetune_SA_ll_SA_SP'): 'FT+SA+SP',
    ('canary-finetune_SA_SP_ll_PS_SP', 'parakeet-finetune_SA_ll_PS_SP'): 'FT+PS+SP',
    ('canary-finetune_SA_SP_ll_SA_PS_SP', 'parakeet-finetune_SA_ll_SA_PS_SP'): 'FT+SA+PS+SP',
}
...
def perform_pairwise_comparisons(
    ...
    for dataset in datasets:
        for model_family in models.keys():
            ...
            if save_dir is None:
                save_dir = Path("reports/csr_statistical_analysis/")
...
def pairwise_comparison_pipeline(type: str="speaker"):
    """
    Full pipeline to perform pairwise comparisons and generate heatmaps.
    """
    results_path = Path("reports/metrics/")
    if type == "speaker":
        file_name = "lillelyd_finetune_speaker_stitched_detailed_results.parquet"
    elif type == "sentence":
        file_name = "lillelyd_finetune_sentence_stitched_detailed_results.parquet"
    else:
        raise ValueError(f"Invalid type: {type}. Must be 'speaker' or 'sentence'.")
    logger.info(f"Starting pairwise comparison pipeline for {type} data...")
    ...
    result_paths = perform_pairwise_comparisons(scores_dict, MODELS, datasets, save_dir=Path(f"reports/csr_statistical_analysis_{type}/"))
    ...
    for file in files_to_process:
        #plot_pvalue_heatmap(file, format_dict=expanded, save_dir=Path("reports/csr_statistical_analysis/"))
        plot_pvalue_heatmap(file, format_dict=expanded, save_dir=Path(f"reports/csr_statistical_analysis_{type}/"), lower_triangle_only=True)