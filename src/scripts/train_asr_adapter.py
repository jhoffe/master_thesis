...
@hydra_runner(config_path="../../nemo_config", config_name="canary-linear-finetune.yaml")
def main(cfg):
    ...
    if cfg.model.pretrained_model is not None and cfg.model.nemo_model is not None:
        raise ValueError(
            "Cannot set both `cfg.model.nemo_model` and `cfg.model.pretrained_model`. Select one only."
        )
    ...
    if cfg.model.pretrained_model is not None:
        ...
        model = ASRModel.from_pretrained(
            cfg.model.pretrained_model, override_config_path=model_cfg, trainer=trainer
        )
    ...
    with open_dict(cfg.model.adapter):
        ...
        print(f"Found `{adapter_type}` config :\n{OmegaConf.to_yaml(adapter_type_cfg)}")
